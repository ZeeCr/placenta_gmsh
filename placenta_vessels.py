...
def add_spiral_arteries_and_lobule_veins(model,no_lobules,lobules, \
        cotyledons_shrunk,lobules_shrunk,no_inlets,no_lobule_veins,no_outlets,outlet_faces):
    ...
    for placentone_no in range(0,no_lobules):
        ...
        - math.sqrt(initial_sphere_radius**2 - (placentone_pt_on_circle[0]**2 + placentone_pt_on_circle[1]**2))
        ...
        try:
            ...
            gmsh.model.occ.fillet([max_vol_tag], [fns.find_line_with_centre(model,artery_pt_on_placenta)], [septal_artery_funnel_radius])
        ...
        to_store_stop_at_basal_veins_added = numpy.append(to_store_stop_at_basal_veins_added,stop_at_veins_added)
        to_store_no_vein_to_add = numpy.append(to_store_no_vein_to_add,no_vein_to_add)
        ...
        for vein_no in range(1,no_vein_to_add+1,1):
            ...
            vein_pt_on_placenta[0:2] = vein_pt_on_placenta[0:2]+vein_side*septal_vessel_separation*placenta_tangent[0:2]
            ...
            lobule_nos[0] = fns.find_lobule_no(front_est,lobules_shrunk)
            lobule_nos[1] = fns.find_lobule_no(vein_inside_pt,lobules_shrunk)
            lobule_nos[2] = fns.find_lobule_no(left_est,lobules_shrunk)
            lobule_nos[3] = fns.find_lobule_no(right_est,lobules_shrunk)
            cotyledon_nos[0] = fns.find_cotyledon_no(front_est,cotyledons_shrunk)
            cotyledon_nos[1] = fns.find_cotyledon_no(vein_inside_pt,cotyledons_shrunk)
            cotyledon_nos[2] = fns.find_cotyledon_no(left_est,cotyledons_shrunk)
            cotyledon_nos[3] = fns.find_lobule_no(right_est,cotyledons_shrunk)
...
##################################################################  
def create_marginal_veins(model,cotyledons_shrunk,lobules_shrunk,no_outlets,outlet_faces):
    ...
    while no_marginal_sinus_veins_added < no_veins_to_add and debug_counter < 1e5:
        ...
        lobule_nos[0] = fns.find_lobule_no(left_est,lobules_shrunk)
        lobule_nos[1] = fns.find_lobule_no(cylinder_inside_pt,lobules_shrunk)
        lobule_nos[2] = fns.find_lobule_no(right_est,lobules_shrunk)
        cotyledon_nos[0] = fns.find_cotyledon_no(left_est,cotyledons_shrunk)
        cotyledon_nos[1] = fns.find_cotyledon_no(cylinder_inside_pt,cotyledons_shrunk)
        cotyledon_nos[2] = fns.find_cotyledon_no(right_est,cotyledons_shrunk)
        ...
        if (numpy.linalg.norm(cylinder_inside_pt[0:2]-lobules_shrunk[lobule_nos[0]].centroid[0:2]) < \
